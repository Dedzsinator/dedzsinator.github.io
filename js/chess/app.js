...
function updateBoard() {
  ...
  if (currentColor === "w") {
    updateView(0,0);
    Log("A"+white+"játékos következik");
    if (inCheck) { 
      Log("A"+white+"játékos királya sakkban van");
    }
    if (inCheckmate) { 
      Log("A"+white+"királya sakk mattot kapott, "+black+" nyert !");
    }
  } else {
    updateView(0,180);
    Log("A" + black + "játékos következik");
    if (inCheck) { 
      Log("A" + black + "játékos királya sakkban van");
    }
    if (inCheckmate) { 
      Log("A" + black + "királya sakk mattot kapott, " + white + " nyert !");
    }
  }
}
...
function setTheme(event) {
  ...
  if (data === "classic" || data === "marble" ) { white = "Fehér", black = "Fekete" }
  else if (data === "flat" || data === "wireframe" ) { white = "Kék", black = "Piros" }
}
